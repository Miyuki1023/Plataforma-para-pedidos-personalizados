...
watch(() => props.fotoPerfil, (v) => {
  if (v) localFoto.value = v
})
...
const saveProfile = async () => {
  ...
  try {
    ...
    if (updatedUser) {
      ...
      localFoto.value = updatedUser.foto_perfil || localFoto.value
    }
...
const addAddress = async () => {
  ...
  try {
    const payload = {
      nombre_direccion: newLabel.value || 'Mi dirección',
      direccion: newStreet.value,
      referencia: newRef.value,
    ...
    addrError.value = err.message || 'Error al guardar'
  } finally {
