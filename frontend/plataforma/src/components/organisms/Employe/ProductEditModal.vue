...
watch(
  ...
  async (newProduct) => {
    if (newProduct) {
      ...
      try {
        const resp = await apiService.get(`/productos/${newProduct.id}/options`)
...
async function handleSubmit() {
  ...
  try {
    ...
    const response = await apiService.put(`/productos/${props.product?.id}`, {
    ...
    for (const oldId of originalOptionsIds.value) {
      if (!currentIds.has(oldId)) {
        await apiService.delete(`/productos/${props.product?.id}/options/${oldId}`)
      }
    }
    // 2. Crear o actualizar las actuales
    for (const opt of options.value) {
      if (opt.id) {
        await apiService.put(`/productos/${props.product?.id}/options/${opt.id}`, opt)
      } else if (opt.nombre.trim()) {
        await apiService.post(`/productos/${props.product?.id}/options`, opt)
      }
    }
